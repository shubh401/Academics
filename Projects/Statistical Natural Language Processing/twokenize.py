...
Contractions = \
    re.compile(u"(?i)(\w+)(n['’′]t|['’′]ve|['’′]ll|['’′]d|['’′]re|['’′]s|['’′]m)$"
               , re.UNICODE)
Whitespace = \
    re.compile(u"[\s\u0020\u00a0\u1680\u180e\u202f\u205f\u3000\u2000-\u200a]+"
               , re.UNICODE)
...
commonTLDs = \
    r"(?:com|org|edu|gov|net|mil|aero|asia|biz|cat|coop|info|int|jobs|mobi|museum|name|pro|tel|travel|xxx)"
ccTLDs = \
...
urlStart2 = r"\b(?:[A-Za-z\d-])+(?:\.[A-Za-z0-9]+){0,3}\." \
    + regex_or(commonTLDs, ccTLDs) + r"(?:\." + ccTLDs + r")?(?=\W|$)"
urlBody = r"(?:[^\.\s<>][^\s<>]*?)?"
urlExtraCrapBeforeEnd = regex_or(punctChars, entity) + '+?'
urlEnd = r"(?:\.\.+|[<>]|\s|$)"
url = regex_or(urlStart1, urlStart2) + urlBody + '(?=(?:' \
...
standardAbbreviations = \
    r"\b(?:[Mm]r|[Mm]rs|[Mm]s|[Dd]r|[Ss]r|[Jj]r|[Rr]ep|[Ss]en|[Ss]t)\."
arbitraryAbbrev = regex_or(aa1, aa2, standardAbbreviations)
separators = "(?:--+|―|—|~|–|=)"
decorations = \
    u"(?:[♫♪]+|[★☆]+|[♥❤♡]+|[\u2639-\u263b]+|[\ue001-\uebbb]+)"
thingsThatSplitWords = r"[^\s\.,?\"]"
embeddedApostrophe = thingsThatSplitWords + r"+['’′]" \
    + thingsThatSplitWords + '*'
...
basicface = '(?:' + bfLeft + bfCenter + bfRight + ')|' + s3 + '|' + s4 \
    + '|' + s5
...
emoticon = regex_or('(?:>|&gt;)?' + regex_or(normalEyes, wink)
                    + regex_or(noseArea, '[Oo]') + regex_or(tongue
                    + r"(?=\W|$|RT|rt|Rt)", otherMouths
                    + r"(?=\W|$|RT|rt|Rt)", sadMouths, happyMouths),
                    regex_or('(?<=(?: ))', '(?<=(?:^))')
                    + regex_or(sadMouths, happyMouths, otherMouths)
                    + noseArea + regex_or(normalEyes, wink)
                    + '(?:<|&lt;)?', eastEmote.replace('2', '1', 1),
                    basicface, oOEmote)  # Standard version  :) :( :] :D :P
...
Protected = re.compile(regex_or(  # numNum,
    Hearts,
    url,
    Email,
    timeLike,
...
EdgePunctLeft = re.compile(offEdge + '(' + edgePunct + '+)('
                           + notEdgePunct + ')', re.UNICODE)
EdgePunctRight = re.compile('(' + notEdgePunct + ')(' + edgePunct + '+)'
                             + offEdge, re.UNICODE)
...
def normalizeTextForTagger(text):
    assert sys.version_info[0] >= 3 and sys.version_info[1] > 3, \
        'Python version >3.3 required'
    text = text.replace('&amp;', '&')
...
def tokenizeText(text):
    return ' '.join(tokenizeRawTweetText(text))
