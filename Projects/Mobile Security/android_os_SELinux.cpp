...
/*
*Function: computeContextCreate
*Purpose: compute default context for new object
*Parameters: subjectContext: Context of the creator
*objectContext: Context of the parent object of the new object
*objectClass: Class of the new object
*Returns: The jstring of the default security context for the new object (or null in case of error)
*/
static jstring computeContextCreate(JNIEnv *env, jobject,
                                    jstring subjectContextStr, jstring objectContextStr,
                                    jstring objectClassStr) {
    if (isSELinuxDisabled) {
        return NULL;
    }
    char *ctx;
    ...
    ScopedUtfChars subjectContext(env, subjectContextStr);
    if(subjectContext.c_str() == NULL) {
        ALOGV("Error in security_compute_create: subjectContext");
        return NULL;
    }
    ScopedUtfChars objectContext(env, objectContextStr);
    if(objectContext.c_str() == NULL) {
        ALOGV("Error in security_compute_create: objectContext");
        return NULL;
    }
    ScopedUtfChars objectClass(env, objectClassStr);
    if(objectClass.c_str() == NULL) {
        ALOGV("Error in security_compute_create: objectClass");
        return NULL;
    }
    char *tmp1 = const_cast<char *>(subjectContext.c_str());
    char *tmp2 = const_cast<char *>(objectContext.c_str());
    char *tmp3 = const_cast<char *>(objectClass.c_str());
    if ((sclass = string_to_security_class(tmp3)) == 0) return NULL;
    if (security_compute_create(tmp1, tmp2, sclass, &ctx) < 0) {
        ALOGV("Error in security_compute_create");
        return NULL;
    }
    ScopedLocalRef<jstring> securityString(env, NULL);
    securityString.reset(env->NewStringUTF(ctx));
    ALOGV("computeContextCreate(%s, %s, %s) => %s", subjectContext.c_str(), objectContext.c_str(), objectClass.c_str(), ctx);
    freecon(ctx);
    return securityString.release();
}
...
static const JNINativeMethod method_table[] = {
    ...
    { "computeContextCreate"     , "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;", (void*)computeContextCreate},
};
