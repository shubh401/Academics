...
public class ClipboardService extends IClipboard.Stub {
    ...
    void setPrimaryClipInternal(PerUserClipboard clipboard, ClipData clip) {
        ...
        clipboard.securityContext = SELinux.getPidContext(Binder.getCallingPid());
        clipboard.objectContext = getSecurityContext(clipboard.securityContext);
        ...
        try {
            for (int i = 0; i < n; i++) {
                try {
                    ...
                    if ((clipboard.securityContext.equals(li.securityContext)) || (li.securityContext.contains("corp_app"))) {
    ...
    public ClipData getPrimaryClip(String pkg) {
        String securityContext = getSecurityContext(SELinux.getPidContext(Binder.getCallingPid()));
        synchronized (this) {
            if(!getClipboard().objectContext.contains(securityContext)) {
                ClipData emptyClipData = new ClipData("", new String[] {ClipDescription.MIMETYPE_TEXT_PLAIN}, new ClipData.Item(""));
                return emptyClipData;
            }
    ...
    public ClipDescription getPrimaryClipDescription(String callingPackage) {
        synchronized (this) {
            String securityContext = getSecurityContext(SELinux.getPidContext(Binder.getCallingPid()));
            if(!getClipboard().objectContext.contains(securityContext)) {
                ClipDescription emptyDescription = new ClipDescription("", new String[] {ClipDescription.MIMETYPE_TEXT_PLAIN});
    ...
    public void addPrimaryClipChangedListener(IOnPrimaryClipChangedListener listener,
            ...
            String securityContext = SELinux.getPidContext(Binder.getCallingPid());
        synchronized (this) {
            ListenerInfo listenerInfo = new ListenerInfo(Binder.getCallingUid(), callingPackage);
            listenerInfo.securityContext = securityContext;
            getClipboard().primaryClipListeners.register(listener, listenerInfo);
        }
    }
    ...
    public boolean hasClipboardText(String callingPackage) {
        synchronized (this) {
            ...
            Slog.i(TAG, "hasClipboardText=" + getClipboard().objectContext);
    ...
    private final String getSecurityContext(String securityContext) {
        String defaultSecurityContext = SELinux.computeContextCreate(securityContext, getClipboard().securityContext, "clipservice");
